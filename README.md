...
```
![Airline Market Share Pie Chart](screenshots/1_Altitude_Drop.png)

### 🎯 Deep-Dive Case Studies: Out-of-Order Ingestion & Tactical Flight Profiles

To prove the pipeline's capacity for advanced data validation, a deep-dive analysis was conducted on two extreme anomalies flagged by the KQL engine. These edge cases demonstrate the critical intersection between real-world operational realities and time-series data quality.

#### Case Study 1: Out-of-Order Packet Ingestion (Flight ADB797F)
The query flagged a dramatic vertical drop from **8,168m down to 464m** within a consecutive sequence for flight **ADB797F**—a heavy-cargo aircraft operated by **Antonov Airlines** departing from Rzeszów-Jasionka Airport (RZE/EPRZ), a prominent global tactical logistics hub.

* **Technical Root Cause:** This event represents a classic **Out-of-Order Packet Ingestion** anomaly caused by network jitter in the crowdsourced ADS-B receiver network. The telemetry vector at 464m was physically generated by the aircraft immediately post-takeoff. However, due to network routing delays, this packet arrived at the ingestion endpoint *after* the 8,168m vector (which was captured right before the aircraft exited local ground receiver coverage). 
* **Data Engineering Takeaway:** This scenario perfectly validates why real-time streaming architectures cannot implicitly trust the physical arrival order of data logs. It enforces the mandatory design pattern of applying a strict chronological sort (`order by timestamp asc`) prior to executing any stateful windowing operations.
